...
class FolderProvider with ChangeNotifier {
  ...
  Future<void> fetchFolder({String? code}) async {
    QuerySnapshot querySnapshot =
        await PocketPalFirestore().getFolderSnapshot(orderBy: _orderBy ?? "folderDate", code: code);

    _folderList = querySnapshot.docs.map((doc) => Folder.fromMap(doc.data() as Map<String, dynamic>)).toList();
    notifyListeners();
    return;
  }

  Future<void> createFolder(Map<String, dynamic> data, {String? code}) async {
    await PocketPalFirestore().addFolder(data, code: code);
    fetchFolder(code: code);
    return;
  }

  Future<void> updateFolder(Map<String, dynamic> data, String docName, {String? code}) async {
    await PocketPalFirestore().updateFolder(docName, data, code: code);
    fetchFolder(code: code);
    return;
  }
