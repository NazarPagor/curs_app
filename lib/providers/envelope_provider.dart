...
class EnvelopeProvider with ChangeNotifier {
  ...
  Future<void> fetchEnvelope(String docName, {String? code}) async {
    QuerySnapshot querySnapshot =
        await PocketPalFirestore().getEnvelopeSnapshot(docName, orderBy: _orderBy ?? "envelopeDate", code: code);
    _envelopeList = querySnapshot.docs.map((doc) => Envelope.fromMap(doc.data() as Map<String, dynamic>)).toList();
    notifyListeners();
    return;
  }

  Future<void> createEnvelope(Map<String, dynamic> data, String docName, {String? code}) async {
    await PocketPalFirestore().addEnvelope(data, docName, code: code);
    fetchEnvelope(docName, code: code);
    return;
  }

  Future<void> updateEnvelope(Map<String, dynamic> data, String docName, String docId, {String? code}) async {
    await PocketPalFirestore().updateEnvelope(docName, docId, data, code: code);
    fetchEnvelope(docName, code: code);
    return;
  }

  Future<void> deleteEnvelope(String docName, String docId, {String? code}) async {
    await PocketPalFirestore().deleteEnvelope(docName, docId, code: code);
    fetchEnvelope(docName, code: code);
    return;
  }
